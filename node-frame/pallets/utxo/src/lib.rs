...
impl<T: Trait> Module<T> {
    ...
    fn validate_signed(
        ...
        account: Option<T::AccountId>,
            ...
            tx.len() <= MAX_INPUTS.min(MAX_OUTPUTS),
            Error::<T>::TransactionTooBig
        );
        ...
        for (pair_idx, pair) in tx.iter().enumerate() {
            ...
            for hash in pair.input.iter() {
                if let Some(input_utxo) = UtxoStore::<T>::get(&hash) {
                        ...
                        skip_ownership || Some(input_utxo.pubkey) == account,
                        Error::<T>::NotOwner
                    );
            ...
            for (output_idx, output) in pair.output.iter().enumerate() {
                ...
                pair_input >= pair_output,
                Error::<T>::OutputGreaterThanInput
            );
        }
