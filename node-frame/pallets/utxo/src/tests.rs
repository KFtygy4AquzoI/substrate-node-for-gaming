...
const BOB_PHRASE: &str =
    "toast youth kingdom soft caution hand burst cliff scissors wisdom test letter";
const EVE_PHRASE: &str =
    "puzzle thing true express thumb hidden spring series render earn chimney essay";
const KARL_PHRASE: &str =
    "monitor exhibit resource stumble subject nut valid furnace obscure misery satoshi assume";
const GENESIS_UTXO: [u8; 32] =
...
fn new_test_ext_and_keys() -> (sp_io::TestExternalities, BTreeMap<&'static str, Public>) {
    ...
    let alice_pub_key = keystore
        .write()
        .sr25519_generate_new(SR25519, Some(ALICE_PHRASE))
        .unwrap();
    let bob_pub_key = keystore
        .write()
        .sr25519_generate_new(SR25519, Some(BOB_PHRASE))
        .unwrap();
    let eve_pub_key = keystore
        .write()
        .sr25519_generate_new(SR25519, Some(EVE_PHRASE))
        .unwrap();
    let karl_pub_key = keystore
        .write()
        .sr25519_generate_new(SR25519, Some(KARL_PHRASE))
        .unwrap();
    ...
    T: Into<H256>,
{
    ...
    assert_ok!(Utxo::send_transaction(
...
fn transfer_works() {
    ...
    test_ext.execute_with(|| {
        ...
        assert_ok!(Utxo::transfer(
            Origin::signed(alice_pub_key.into()),
            vec![genesis_utxo_hash],
            bob_pub_key.into()
        ));
